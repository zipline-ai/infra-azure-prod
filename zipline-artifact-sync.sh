#!/bin/bash
set -euxo pipefail

function print_usage() {
    echo "Usage: $0 [OPTIONS]"
    echo "Options:"
    echo "  --artifact_prefix <azure storage container>     Specify the azure storage container to upload artifacts to e.g. \"https://ziplineai2.blob.core.windows.net/dev-zipline-artifacts\""
    echo "  --version <version>                 Specify the version you want to run"
    echo "  --no_set_latest                     Set to not set the version as latest"
    echo "  -h, --help  Show this help message"
}

if [ $# -ne 4 ]; then
    print_usage
    exit 1
fi

SET_LATEST=true

while [[ $# -gt 0 ]]; do
    case $1 in
        --artifact_prefix)
            if [[ -z $2 ]]; then
                echo "Error: --artifact_prefix requires a value"
                print_usage
                exit 1
            fi
            ARTIFACT_PREFIX="$2"
            shift 2
            ;;
        -h|--help)
            print_usage
            exit 0
            ;;
        --version)
            if [[ -z $2 ]]; then
                echo "Error: --version requires a value"
                print_usage
                exit 1
            fi
            VERSION="$2"
            shift 2
            ;;
        --no_set_latest)
            SET_LATEST=false
            ;;
        *)
            echo "Unknown option: $1"
            print_usage
            exit 1
            ;;
    esac
done

# Login to artifactory
jf c add --url https://ziplineai.jfrog.io ziplineai --overwrite

# Download jars from artifactory
jf rt dl "jars/$VERSION/*" ./jars/

# Download wheels from artifactory
jf rt dl "wheels/$VERSION/*" ./wheels/

# Download dataproc initialization scripts from artifactory
jf rt dl "scripts/*" ./scripts/

echo "Beginning Upload To Azure Storage..."

# Upload jars to Azure Storage
az storage blob upload-batch \
    -d ${ARTIFACT_PREFIX%/} \
    --destination-path /release/$VERSION/jars/ \
    -s ./jars/$VERSION/ \
    --auth-mode login \
    --overwrite true


# Upload wheel to Azure Storage
az storage blob upload-batch \
    -d ${ARTIFACT_PREFIX%/} \
    --destination-path /release/$VERSION/wheels/ \
    -s ./wheels/$VERSION/ \
    --auth-mode login \
    --overwrite true

# Upload scripts to Azure Storage
az storage blob upload-batch \
    -d ${ARTIFACT_PREFIX%/} \
    --destination-path /scripts/ \
    -s ./scripts/ \
    --auth-mode login \
    --overwrite true

if [ $SET_LATEST = true ]; then

 # Upload jars to Azure Storage
 az storage blob upload-batch \
     -d ${ARTIFACT_PREFIX%/} \
     --destination-path /release/latest/jars/ \
     -s ./jars/$VERSION/ \
     --auth-mode login \
     --overwrite true


 # Upload wheel to Azure Storage
 az storage blob upload-batch \
     -d ${ARTIFACT_PREFIX%/} \
     --destination-path /release/latest/wheels/ \
     -s ./wheels/$VERSION/ \
     --auth-mode login \
     --overwrite true

fi
# Cleanup
trap 'rm -rf ./jars ./wheels ./scripts' EXIT